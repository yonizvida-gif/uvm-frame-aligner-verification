...
flowchart TB
    ...
    DRV -->|rx_data| DUT
    DRV -. input stream .-> MONIN
    DUT -->|fr_byte_position, frame_detect| MONOUT
    IF -.-> DRV
    IF -.-> MONIN
    IF -.-> MONOUT
    IF -.-> DUT
...
The input and output monitors publish transactions independently. Both monitor streams are consumed in parallel by the scoreboard and the functional coverage component.
